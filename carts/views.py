...
def add_cart(request, product_id):
    product = Product.objects.get(
        id=product_id
    )  # get the product having id = product_id (add cart button of the product i clicked))
    product_variation = (
        []
    )  # empty list which will contain all the variations of the product
    if request.method == "POST":  # capture the POST request
        for item in request.POST:  # looping the data present in POST request
            key = item
            value = request.POST[
                key
            ]  # example: color=red and size=XXL (key will be the "color" and value will be "red")
            ...
            try:
                ...
                product_variation.append(
                    variation
                )  # append the object in list product_variation
            except:
                pass
    ...
    try:
        ...
        Cart.DoesNotExist
    ):  # We are checking if cart with our session_key is present or not, if it's present then we will just store it in cart variable otherwise create it
        cart = Cart.objects.create(
            cart_id=_cart_id(request),
        )
        cart.save()  # save the cart

    product_exist_in_cart = CartItem.objects.filter(
        product=product, cart=cart
    ).exists()  # check if the selected product is present in our cart or not
    ...
    if product_exist_in_cart:  # if product present
        ...
        for item in cart_item:
            existing_variation = (
                item.variations.all()
            )  # store the existing variations of a product
            ex_var_list.append(
                list(existing_variation)
            )  # append the list of existing_variation of product in "ex_var_list"
            id.append(
                item.id
            )  # append id of each variation of the product in list "id"

        if (
            product_variation in ex_var_list
        ):  # if product_variation is present in the ex_var_list
            index = ex_var_list.index(
                product_variation
            )  # index of the product_variation present in ex_var_list
            item_id = id[index]  # get the id of that variation
            item = CartItem.objects.get(
                product=product, id=item_id
            )  # get the product having that id
            item.quantity += 1  # increase quantity by 1
            item.save()  # save
        else:  # if product_varition is not present in the ex_var_list
            item = CartItem.objects.create(
                product=product, quantity=1, cart=cart  # create a new item in the cart
            )
        ...
        cart_item.variations.add(
            *product_variation
        )  # add the product_variations to the cart_item and save it
        cart_item.save()
